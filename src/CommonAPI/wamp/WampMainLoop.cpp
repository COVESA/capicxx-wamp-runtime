...
WampMainLoop::WampMainLoop(std::shared_ptr<MainLoopContext> context)
        : context_(context),
    ...
    dispatchSourceListenerSubscription_ = context_->subscribeForDispatchSources(
            std::bind(&WampMainLoop::registerDispatchSource, this,
                    std::placeholders::_1, std::placeholders::_2),
            std::bind(&WampMainLoop::unregisterDispatchSource,
                    this, std::placeholders::_1));
    watchListenerSubscription_ = context_->subscribeForWatches(
            std::bind(&WampMainLoop::registerWatch, this,
                    std::placeholders::_1, std::placeholders::_2),
            std::bind(&WampMainLoop::unregisterWatch, this,
                    std::placeholders::_1));
    timeoutSourceListenerSubscription_ = context_->subscribeForTimeouts(
            std::bind(&WampMainLoop::registerTimeout, this,
                    std::placeholders::_1, std::placeholders::_2),
            std::bind(&WampMainLoop::unregisterTimeout, this,
                    std::placeholders::_1));
    wakeupListenerSubscription_ = context_->subscribeForWakeupEvents(
            std::bind(&WampMainLoop::wakeup, this));
}

WampMainLoop::~WampMainLoop() {
...
void WampMainLoop::run(const int64_t& timeoutInterval) {
	std::cout << "WampMainLoop::run called" << std::endl;
    while (!hasToStop_) {
        doSingleIteration(timeoutInterval);
    }
	std::cout << "WampMainLoop::run: finished" << std::endl;
}

void WampMainLoop::stop() {
    hasToStop_ = true;
    wakeup();
}

void WampMainLoop::doSingleIteration(const int64_t& timeout) {
	static int i = 0;
	i++;
	std::cout << "WampMainLoop::doSingleIteration: " << i << std::endl;
...
bool WampMainLoop::prepare(const int64_t& timeout) {
...
void WampMainLoop::wakeup() {
    ...
    if(::write(wakeFd_.fd, &wake, sizeof(int64_t)) == -1) {
        std::perror("WampMainLoop::wakeup");
    }
#endif
}

void WampMainLoop::wakeupAck() {
#ifdef WIN32
...
void WampMainLoop::registerFileDescriptor(
        const WampMainLoopPollFd& fileDescriptor) {
    std::lock_guard<std::mutex> itsLock(fileDescriptorsMutex_);
    managedFileDescriptors_.push_back(fileDescriptor);
}

void WampMainLoop::unregisterFileDescriptor(
        const WampMainLoopPollFd& fileDescriptor) {
    wakeup();
    std::lock_guard<std::mutex> itsLock(fileDescriptorsMutex_);
...
void WampMainLoop::registerDispatchSource(DispatchSource* dispatchSource,
...
void WampMainLoop::unregisterDispatchSource(DispatchSource* dispatchSource) {
    {
        std::lock_guard<std::mutex> itsLock(dispatchSourcesMutex_);
        for (auto dispatchSourceIterator = registeredDispatchSources_.begin();
            dispatchSourceIterator != registeredDispatchSources_.end();
...
void WampMainLoop::registerWatch(Watch* watch,
        const DispatchPriority dispatchPriority) {

    std::lock_guard<std::mutex> itsLock(watchesMutex_);
    WampMainLoopPollFd fdToRegister = watch->getAssociatedFileDescriptor();
...
void WampMainLoop::unregisterWatch(Watch* watch) {
...
void WampMainLoop::registerTimeout(Timeout* timeout,
        const DispatchPriority dispatchPriority) {
...
void WampMainLoop::unregisterTimeout(Timeout* timeout) {
    {
        std::lock_guard<std::mutex> itsLock(timeoutsMutex_);
        for (auto timeoutIterator = registeredTimeouts_.begin();
                timeoutIterator != registeredTimeouts_.end();
