#ifndef COMMONAPI_WAMP_MAINLOOP_HPP_
#define COMMONAPI_WAMP_MAINLOOP_HPP_
...
typedef pollfd WampMainLoopPollFd;

class WampMainLoop {
 public:
    WampMainLoop() = delete;
    WampMainLoop(const WampMainLoop&) = delete;
    WampMainLoop& operator=(const WampMainLoop&) = delete;
    WampMainLoop(WampMainLoop&&) = delete;
    WampMainLoop& operator=(WampMainLoop&&) = delete;

    COMMONAPI_EXPORT explicit WampMainLoop(std::shared_ptr<MainLoopContext> context);
    COMMONAPI_EXPORT ~WampMainLoop();
 ...
 private:
    ...
    COMMONAPI_EXPORT void registerFileDescriptor(const WampMainLoopPollFd& fileDescriptor);
    COMMONAPI_EXPORT void unregisterFileDescriptor(const WampMainLoopPollFd& fileDescriptor);
    ...
    std::vector<WampMainLoopPollFd> managedFileDescriptors_;
    ...
    WampMainLoopPollFd wakeFd_;

#ifdef WIN32
    WampMainLoopPollFd sendFd_;
#endif
...
#endif // COMMONAPI_WAMP_MAINLOOP_HPP_
